...
@pytest.mark.asyncio
async def test_org_quality_analytics_requires_auth(client: AsyncClient) -> None:
    response = await client.get("/api/v1/analytics/org/quality")
    assert response.status_code in {401, 403}


@pytest.mark.asyncio
async def test_admin_quality_analytics_requires_auth(client: AsyncClient) -> None:
    response = await client.get("/api/v1/analytics/admin/quality")
    assert response.status_code in {401, 403}
